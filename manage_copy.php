...
from BOOK, BOOK_COPY
WHERE BOOK.BOOKID = BOOK_COPY.PhyBookID AND BOOK_COPY.Availibility = 'True'; ";
...
$query = "
select BOOK.BOOKID,CopyID,Title 
from BOOK, BOOK_COPY
WHERE BOOK.BOOKID = BOOK_COPY.PhyBookID AND BOOK_COPY.Availibility = 'True'
ORDER BY BOOKID  OFFSET $page_first_result ROWS FETCH NEXT $results_per_page ROWS ONLY;";
...
<section class="empty__page">
    <div class="d-flex flex-column px-5 my-2 gap-1">
        <h1 class="text-center">Quản lý Sách Copy</h1>
        <form class="d-flex px-5" role="search" method="get" action="manage_copy-search.php">
                ...
                ?>
                    <tr>
                        ...
                        <td class="d-flex p-1 gap-1">
                            <form action="manage_copy-insert.php" method="post" class="w-50">
                                <input type="hidden" name="id" value="<?= $row["BOOKID"] ?>">
                                <input type="hidden" name="c_id" value="<?= $row["CopyID"] ?>">
                ...
                <ul class="pagination">
                    <li class="page-item">
                        <a class="page-link <?php if ($page_cur <= 1) {
                                                echo 'disabled';
                                            } ?>" href="manage_copy.php?page=<?= $page_cur - 1 ?>" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <li class="page-item">
                        <a class="page-link active" href="manage_copy.php?page=<?= $page_cur ?>">
                            <?= $page_cur ?>
                        </a>
                    </li>
                    <?php
                    ...
                    ?>
                        <li class="page-item">
                            <a class="page-link" href="manage_copy.php?page=<?= $page_cur + 1 ?>">
                                <?= $page_cur + 1 ?>
                            </a>
                        </li>
                    <?php
                    ...
                    ?>
                        <li class="page-item">
                            <a class="page-link" href="manage_copy.php?page=<?= $page_cur + 2 ?>">
                                <?= $page_cur + 2 ?>
                            </a>
                        </li>
                    <?php
                    ...
                    <li class="page-item">
                        <a class="page-link <?php if ($page_cur >= $number_of_page) {
                                                echo 'disabled';
                                            } ?>" href="manage_copy.php?page=<?= $page_cur + 1 ?>" aria-label="Next">
